...
void GeoTagControllerTest::_geoTagControllerTest()
{
    ...
    QSignalSpy spyGeoTagControllerInProgress(controller, &GeoTagController::progressChanged);
    QSignalSpy spyGeoTagControllerError(controller, &GeoTagController::progressChanged);

    controller->startTagging();

    QCOMPARE(spyGeoTagControllerInProgress.wait(5000), true);
    QCOMPARE(spyGeoTagControllerProgress.wait(5000), true);

    controller->cancelTagging();

    QCOMPARE(spyGeoTagControllerError.wait(5000), true);

    QVERIFY(!controller->inProgress());
    QVERIFY(controller->progress() > 0);
    QCOMPARE(controller->errorMessage(), QStringLiteral("Tagging cancelled"));
}
