...
LogReplayLinkConfiguration::LogReplayLinkConfiguration(LogReplayLinkConfiguration *copy, QObject *parent)
...
bool LogReplayLink::_connect()
{
    if (qgcApp()->toolbox()->multiVehicleManager()->activeVehicle()) {
        _replayError(QStringLiteral("You must close all connections prior to replaying a log."));
        return false;
    }
